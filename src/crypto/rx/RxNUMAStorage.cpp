...
{
    XMRIG_LOG_WARN("%s" CYAN_BOLD("#%u ") RED_BOLD("skipped") YELLOW(" (%s)"), rx_tag(), nodeId, reason);
}


static inline void printDatasetReady(uint32_t nodeId, uint64_t ts)
{
    XMRIG_LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("dataset ready") BLACK_BOLD(" (%" PRIu64 " ms)"), rx_tag(), nodeId, Chrono::steadyMSecs() - ts);
}
...
public:
    ...
    inline void createDatasets(bool hugePages)
    {
        ...
        if (m_datasets.empty()) {
            ...
            XMRIG_LOG_WARN(CLEAR "%s" YELLOW_BOLD_S "failed to allocate RandomX datasets, switching to slow mode" BLACK_BOLD(" (%" PRIu64 " ms)"), rx_tag(), Chrono::steadyMSecs() - ts);
        }
        else {
...
private:
    ...
    void printAllocStatus(RxDataset *dataset, uint32_t nodeId, uint64_t ts)
    {
        ...
        XMRIG_LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("allocated") CYAN_BOLD(" %zu MiB") " huge pages %s%3.0f%%" CLEAR BLACK_BOLD(" (%" PRIu64 " ms)"),
                 rx_tag(),
                 nodeId,
                 dataset->size() / oneMiB,
    ...
    void printAllocStatus(RxCache *cache, uint32_t nodeId, uint64_t ts)
    {
        ...
        XMRIG_LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("allocated") CYAN_BOLD(" %4zu MiB") " huge pages %s%3.0f%%" CLEAR " %sJIT" BLACK_BOLD(" (%" PRIu64 " ms)"),
                 rx_tag(),
                 nodeId,
                 cache->size() / oneMiB,
                 (pages.first == pages.second ? GREEN_BOLD_S : RED_BOLD_S),
    ...
    void printAllocStatus(uint64_t ts)
    {
        ...
        XMRIG_LOG_INFO("%s" CYAN_BOLD("-- ") GREEN_BOLD("allocated") CYAN_BOLD(" %4zu MiB") " huge pages %s%3.0f%% %u/%u" CLEAR BLACK_BOLD(" (%" PRIu64 " ms)"),
                 rx_tag(),
                 memory / oneMiB,
